            ...
            <div class="container-fluid">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Sửa Theme</h3>
                    </div>
                    <form action="{{ url('/dashboard/update/'. $theme->id )}}" method="POST" enctype="multipart/form-data">
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="description" class="form-control" id="description" value="{{ $theme->description }}" required>{{ $theme->description }}</textarea>
                            </div>
                            <div class="form-group">
                                <label for="thumbnail">Hình ảnh</label>
                                <input type="file" name="thumbnail" class="form-control" id="thumbnail" required>
                                <p><img src="{{ url('storage/' . $theme->thumbnail) }}" alt="{{ $theme->name }}" width="80" height="80"></p>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <select name="category_id" class="form-control" id="category_id" required>
                                    @foreach($categories as $cate)
                                        <option value="{{ $cate->id }}" {{ $theme->category_id == $cate->id ? 'selected' : '' }}>{{ $cate->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
